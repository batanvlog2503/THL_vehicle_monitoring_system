...
OCR_EVERY_N         = 33      # OCR biển mỗi N frame
SPEED_LIMIT_DEFAULT = 60

# OCR config (LPR v2)
MIN_SCORE_ACCEPT  = 3.5
CROP_PAD_RATIO_X  = 0.15
CROP_PAD_RATIO_Y  = 0.20
MIN_PLATE_H       = 40
UPSCALE_TARGET_H  = 200
...
class PlateCache:
    ...
    def best(self, tid: int) -> dict | None:
        ...
        best_text, data = max(
            agg.items(),
            key=lambda kv: (kv[1]["count"], kv[1]["score_sum"], kv[1]["best_score"])
        )
    ...
    try:
        ...
        for tid, rec in summary.items():
            ...
            if best and best["text"]:
                ...
                # ← THÊM SỬA 3 VÀO ĐÂY
        for d in detections_all:
            best = plate_cache.best(d["id"])
            if best and best["text"]:
                d["plate"] = best["text"]
            if d.get("speed") is not None and d["speed"] > speed_limit:
                d["status"] = "violation"
