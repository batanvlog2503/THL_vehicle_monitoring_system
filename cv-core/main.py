    ...
    try:
        ...
        while True:
            ...
            if frame_idx % OCR_EVERY_N == 0 and vehicle_boxes:
                ...
                for tid, (vx1, vy1, vx2, vy2) in vehicle_boxes.items():
                    ...
                    # Nếu đã có plate tốt rồi thì bỏ qua
                    best_plate = plate_cache.best(int(tid))

                    if best_plate and best_plate["conf"] > 0.85:
                        continue
        ...
        for d in detections_all:
            ...
            if tid not in summary:
                ...
                if d.get("plate"):
                    if (
                            not current.get("plate")
                            or len(d["plate"]) > len(current["plate"])
                    ):
                ...
                if d.get("speed") is not None:
                    if (
                            current.get("speed") is None
                            or d["speed"] > current["speed"]
                    ):
